 ...
 *
 * ДЕМОНСТРАЦІЙНИЙ РЕЖИМ ПОЗНАЧАЄТЬСЯ ЗАВЖДИ. У продукті, де вся цінність
 * тримається на довірі до цитат, показати вигадані дані як справжні гірше,
 * ніж не показати нічого.
 *
 * До запакованого застосунку цей режим більше не доходить узагалі: там
 * `probeBackend` чекає на sidecar до дедлайну оболонки, а якщо не дочекався —
 * показує помилку старту (`components/Startup.tsx`), а не приклад. Смуга й
 * бейдж лишаються для розробки інтерфейсу та `VITE_API_MODE=demo`.
 */
